...
// stem documentation https://katex.org/docs/supported#gdef
== Introduction
...
A drawing of how the device could look like.  Here we try to visualize the different components and how they interact with each other.

.Drawing of the most important components
image::idea.svg[Interactive,800,opts=inline,align=center]

#### specification

.Container sizes

To determine the suitable container size, I conducted an experiment. Initially, I placed a bowl on a scale, calibrated the scale to zero, and then filled the bowl with water. The result was that the bowl held precisely 2000 grams of water.
Following that, I repeated the process using Huni's pet food and found that the bowl had a capacity of 990 grams of food.

The fundamental idea here is that by dividing the weight of the food by the weight of the water in a container of the same size, we are essentially calculating the food's density relative to water within that specific container. This calculation assists in determining the necessary container volume.

We acknowledge that to store one liter of water, approximately 1000 cubic centimeters (cm³) of space are required.

I've set the requirement that the storage container must be capable of holding at least a week's worth of food.
Huni is fed twice per day, with each serving weighing 170 grams, totaling 340 grams per day. Therefore, the weekly volume required is 2380 grams/week.

To be conservative, we'll round that up to 4000 grams to accommodate for larger animals. In other words, we need the storage container to accommodate 4000 grams of food. To store this 4000 grams of food, a container with a volume of 8080 cm³ is required.

Since we will be replenishing the food 14 times a week, the portion container must have a minimum volume of 577 cm³.

The table below provides a breakdown of the calculations used to obtain the values mentioned in the preceding text.

.Volume calculations
[cols=",,", options="header"]
|=== 
| Property                      | Formula                                      | Calculation
| Food density                  | stem:[Fd = "food" / "water" ]                | stem:[{990  g}  / {2000 g}  =0.495 " g/cm"^3]
| Food factor                   | stem:[Ff = "water" / "food" ]                | stem:[{2000 g}  / { 990 g}  \approx "2.02 g/cm"^3]
| Huni's weekly portion         | stem:[ "Mpw" = "portion" * "rate" * "days" ] | stem:[170g * 2 * 7 = "2380 g"]
| Required storage mass         | stem:[ "Mw" ]                                | stem:["4000" g]
| Storage container volume      | stem:[ "Vs" = "Mw" * "Ff" ]                  | stem:["4000 * 2.02 = 8080 cm"^3]
| Portion container volume      | stem:[ "Vp" = "Vs" / ( rate * days ) ]       | stem:[8080 / (2 * 7) \approx 577 " cm"^3]
| Example storage container box | stem:[ "Ves" = "width" * "height" * "depth" ] | stem:[40 cm * 17 cm * 12 cm = 8160 cm^3 ]
| Example portion container box | stem:[ "Vep" = "width" * "height" * "depth" ] | stem:[6 cm * 8 cm * 12 cm = 576 cm^3 ]
|=== 
...
| Huni      | My dogs name is Huni and he is of the type Border Collie.| Knows all a pet knows.
...
The primary function of the device is to accurately dispense the appropriate amount of food into a pet's bowl. 
Achieving this requires us to address two key aspects: measuring each food portion and effectively transferring it into the pet's bowl.

To facilitate the seamless movement of food from the storage container to the portion container, we employ an auger. 
The auger operates as a screw conveyor, allowing for a gradual transfer of food from the storage container to the portion container.
This controlled process enables us to measure the exact quantity of food added to the portion container and stop the auger when the desired amount is reached.

Once this is accomplished, the food can be dispensed from the portion container into the pet's bowl.

=== Components of the Device

This chapter outlines the various components that will make up the device. In cases where unfamiliar components are involved, thorough research and testing will be conducted to gain a comprehensive understanding of them. The outcomes of this research and testing will be documented in a separate report, which will be linked to from this chapter."
...
a| image::l298NDualBridgeStepperModerDriver.png[image]
|===

* Part in my database: http://parts.guttih.com/parts/view/5bfa9d4fec065a5ac27868fd
* Online tutorial: https://microcontrollerslab.com/stepper-motor-l298n-motor-driver-esp32-tutorial/
